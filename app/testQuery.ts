...
export const testQuery = () => {
  ...
  console.log(config)
  const databaseId = config.otbHistories.database.id
  const containerId = config.otbHistories.container.id
...
async function queryContainer() {
  console.log(`Querying container:\n${config.otbHistories.container.id}`)
...
async function queryContainerValues(value) {
  console.log(`Querying container:\n${config.otbHistories.container.id}`)
  ...
  // Including the partition key value of country in the WHERE filter results in a more efficient query
  const querySpec = {
    query: `SELECT SUM(r.${value}) FROM root r WHERE NOT IS_NULL(r.${value})`,
    parameters: [
      {
        name: '@week',
        value:  "2019-W45",
  ...
  for (var queryResult of results) {
    ...
    let resultString = Object.values(queryResult)[0]
    console.log(`\tQuery returned ${resultString}\n in ${seconds} seconds for parameter ${value}`)
  }
}
